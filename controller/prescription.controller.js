...
exports.createPrescription = async (req, res) => {
  ...
  try {
    ...
    // const addPrescription = await Prescription.create(req.body);
...
exports.getPrescription = async (req, res) => {
  ...
  try {
    const getPrescription = await Prescription.findOne({where:{patientId,doctorId,appointmentId}},{limit:1});
...
exports.getPrescriptionByFollowUp = async (req, res) => {
  console.log(req.body)
  const {patientId,doctorId,appointmentId} = req.query;
try {
  const getPrescription = await Prescription.findAll({where:{patientId,doctorId,appointmentId}});
  res.status(200).json(getPrescription)
} catch (error) {
  res.status(500).json({type:error.name, massage:error.massage})
}
