...
exports.getAvailabilityByDoctorId= async (req, res) => {
    ...
    try {
        const availability = await WeeklyAvailabilities.findOne({where:{doctorId}})
        res.status(200).json(availability)
    } catch (error) {
        res.status(500).json({ type: error.name, massage: error.massage });
    }
...
exports.UpdateAvailability = async(req,res) =>{
  const {doctorId,weekday,id} = req.query
  console.log(doctorId,weekday,id);
try {
  const update = await WeeklyAvailabilities.findOne(
